...
public class Evo {
	...
	public Evo(int genome_length, int pop_size, int numElite, double mutate_probability, int numCrossover, Measure<double[]> measure){
		if(pop_size<1) throw new IllegalArgumentException("Die Populationsgr��e muss positiv sein!");
//		if(mutator == null) throw new IllegalArgumentException("Der Mutationsklasse darf nicht null sein!");
		if(genome_length<2) throw new IllegalArgumentException("Die Anzahl der St�tzstellen muss mindestens 2 sein!");
		if(mutate_probability>=1 && mutate_probability<=0) throw new IllegalArgumentException();
	...
	private void select(){
		...
		/* Computes the upper bounds of the sectors on the wheel of fortune
		 * and saves them in the sectors-Array.
		 * The size of a sector is proportial to its probability, so the following holds:
		 * sector[i+1]-sectors[i] = (1- (double) (rest-i) / (double) rest)*(2.0/(rest+1)
		 * 
		 * Problem: the order of the intervals is always the same!
		 * Problem: Not sure if its a real distribution
		 */
		...
		for(int i=0;i<sectors.length-1;i++){
			if(sectors[i]>sectors[i+1]) throw new RuntimeException("Error");
		}
		
		/*
		 * Selects the individuals.
		 * If the individual is an elite(indicated by the elite-Array) it's directly 
		 * taken in the new population otherwise a wheel of fortune-selection is used
		 */
		for(int i=0;i<pop_size;i++){
	...
	private void mutateNCross(){
		...
		double range;
		/*
		 * Only non-elite can be mutated. The mutation probability is a class variable.
		 * The range of the mutation is bounded by twice of the currentvalue of the gene
		 */
		for(int i=0;i<pop_size;i++){
		...
		/*
		 * Crossovermethod: Shuffle Crossover. The Count of Crossovers is regulated by the class variable
		 * numCrossover.
		 *
		for(int i=0; i<numCrossover;i++){
			//select two non-elite
